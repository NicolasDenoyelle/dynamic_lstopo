...
void usage(const char *name, FILE *where)
{
  ...
  fprintf (where, "  --merge               Do not show levels that do not have a hierarchical impact\n");
...
static void output(struct lstopo_output * loutput, const char * filename, enum output_format output_format){
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_replay_opt){
	output_x11_perf_replay(loutput,filename,replay);
      }
      else if(perf_opt){
	output_x11_perf(loutput, &perf_attributes, filename, monitors);
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
      fprintf(stderr,"Performance module not supported on windows\n");
      return;
    }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
	fprintf(stderr,"Performance module not supported with default console output\n");
	return;
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
      fprintf(stderr,"Performance module not supported with console output\n");
      return;
    }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
	fprintf(stderr,"Performance module not supported with synthetic output\n");
	return;
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
	fprintf(stderr,"Performance module not supported with ascii output\n");
	return;
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
	fprintf(stderr,"Performance module not supported with fig output\n");
	return;
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_replay_opt){
	output_png_perf_replay(loutput,filename,replay);
      }
      else if(perf_opt){
	output_png_perf(loutput, &perf_attributes, filename, monitors);
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_replay_opt){
	output_pdf_perf_replay(loutput,filename,replay);
      }
      else if(perf_opt){
	output_pdf_perf(loutput, &perf_attributes, filename, monitors);
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_replay_opt){
	output_ps_perf_replay(loutput,filename,replay);
      }
      else if(perf_opt){
	output_ps_perf(loutput, &perf_attributes, filename, monitors);
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_replay_opt){
	output_svg_perf_replay(loutput,filename,replay);
      }
      else if(perf_opt){
	output_svg_perf(loutput, &perf_attributes, filename, monitors);
      }
      else
#endif
...
#ifdef HWLOC_HAVE_MONITOR
      if(perf_opt){
	fprintf(stderr,"Performance module not supported with xml output\n");
	return;
      }
      else
#endif
...
main (int argc, char *argv[])
{
  ...
  while (argc >= 1)
	...
	perf_opt = 1;
	perf_attributes.perf_accumulate = 1;
      }
      else if (!strcmp (argv[0], "--perf-whole-machine")){
	perf_attributes.perf_whole_machine = 1;
      }
	...
	perf_opt = 1;
	perf_attributes.refresh_usec = atoll(argv[1]);
	opt = 1;
	chk_perf_opt_compatibility();
      }
  ...
  if(perf_replay_opt){
    ...
    if(!perf_input || monitors==NULL)
      monitors = new_default_Monitors(topology,perf_output,perf_attributes.perf_accumulate);
    if(monitors==NULL){
      fprintf(stderr, "failed to create monitors\n");
      goto out_usagefailure;
    }
    if(lstopo_pid!=0){
      Monitors_watch_pid(monitors,lstopo_pid);
    }
	...
	while(1){
      ...
      perf_attributes.executable = argv[0];
      perf_attributes.exe_args = argv;
      output(&loutput, filename, output_format);
    }
