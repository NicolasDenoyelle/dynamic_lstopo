...
#test if demo can be compiled
hwloc_monitor_demo_happy=yes
AC_CHECK_HEADER([omp.h],[],[demo_happy=no])
AC_OPENMP
...
AC_CHECK_HEADER([ncurses.h],[ncurses_header=0],[hwloc_monitor_demo_happy=no])
AC_CHECK_LIB([ncurses],[initscr],[ncurses_lib=0],[hwloc_monitor_demo_happy=no])

if test "$hwloc_monitor_demo_happy" = "yes" ; then
   AC_DEFINE([HWLOC_HAVE_MONITOR_DEMO], [1], [Define to 1 if you are able to compile the 'monitor demonstration'.])
fi
...
#include <term.h>
    ...
    AC_CONFIG_FILES(
        ...
        hwloc_config_prefix[utils/monitor/demo/Makefile]
