...
static struct lstopo_output          loutput;
static unsigned int                  top           = 0;
static const char *                  filename      = NULL;
static enum output_format            output_format = LSTOPO_OUTPUT_DEFAULT;

static void toto_callback(struct lstopo_custom_box *custom, __attribute__ ((unused)) hwloc_obj_t obj){
    sprintf(custom->info, "toto");
    custom->r = (rand() & 255);
    custom->g = (rand() & 255);
    custom->b = (rand() & 255);
}
...
parse_output_format(const char *name, const char *callname __hwloc_attribute_unused)
{
...
int parse_args(const char * callname, hwloc_topology_t topology, int argc, char *argv[]){
	...
	if (!strcmp (argv[0], "-v") || !strcmp (argv[0], "--verbose")) {
	    ...
	    int all = 0;
	    if(argc < 2) goto failure_usagefailure;
	    colon = strchr(argv[1], ':');
	    if (colon) {
		*colon = '\0';
		...
		else{loutput.force_orient[type] = orient;}
		if(!end){break;}
		tmp = end+1;
	    }
	}
	...
	else if (!strcmp (argv[0], "--no-legend")){loutput.legend = 0;}
	...
	else if (hwloc_utils_lookup_input_option(argv, argc, &opt, &input, &input_format, callname)){
    ...
    if(input){
	if(hwloc_utils_enable_input_format(topology, input, &input_format, loutput.verbose_mode > 1, callname))
	    goto failure_usagefailure;
    }
    ...
    /* If output format was not enforced, look at the filename */
	...
	if (!strcmp (restrictstring, "binding")) {
	    ...
	    if(hwloc_bitmap_sscanf(restrictset, restrictstring) != 0){
		fprintf(stderr, "restrict to %s failed\n", restrictstring);
		goto failure_usagefailure;
	    }
	}
...
int main(int argc, char *argv[]){
    ...
    /* set default options */
    loutput = default_output_options(topology, toto_callback);

    /* Parse command arguments */
    if(parse_args(callname, topology, argc-1, argv+1)) goto failure;
